...
interface SitemapResult {
  ...
  url_count?: number; urls?: SitemapURL[];
  sitemap_count?: number; child_sitemaps?: { loc: string; lastmod: string }[];
  stats?: { total_urls: number; with_lastmod: number; with_priority: number; with_changefreq: number; newest_url_age_days: number | null; oldest_url_age_days: number | null; avg_age_days: number | null; };
...
function ageBadge(days: number | null) {
  ...
  const color = days < 30 ? 'var(--success-color)' : days < 180 ? '#F59E0B' : 'var(--error-color)';
...
export default function SitemapValidatorClient() {
  ...
  async function run() {
    ...
    try {
      ...
      if (!data.found && data.error) setError(data.error);
      else setResult(data);
            ...
            <div className="col-lg-8">
              ...
              <MotionWrapper variant="up" delay={0.1}>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center' }}>
                  <input type="url" value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && !loading && run()} placeholder="https://yourwebsite.com or .../sitemap.xml" disabled={loading}
          ...
          <div className="container">
            <MotionWrapper variant="up">
              <div style={{ padding: '20px 28px', background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.25)', borderRadius: 12, marginBottom: 28, display: 'flex', alignItems: 'center', gap: 12 }}>
                <i className="fa fa-circle-check" style={{ color: 'var(--success-color)', fontSize: '1.1rem' }} />
              ...
              {result.stats && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 16, marginBottom: 28 }}>
                  {[
                    { label: 'Total URLs', value: result.stats.total_urls.toLocaleString() },
                    { label: 'With lastmod', value: `${result.stats.with_lastmod} / ${result.stats.total_urls}` },
                    { label: 'With priority', value: `${result.stats.with_priority} / ${result.stats.total_urls}` },
                    { label: 'Avg content age', value: result.stats.avg_age_days !== null ? `${result.stats.avg_age_days}d` : 'N/A' },
                    { label: 'Newest update', value: result.stats.newest_url_age_days !== null ? `${result.stats.newest_url_age_days}d ago` : 'N/A' },
                    { label: 'Oldest URL', value: result.stats.oldest_url_age_days !== null ? `${result.stats.oldest_url_age_days}d ago` : 'N/A' },
                  ].map(s => (
              ...
              {/* Issues */}
              {result.issues && result.issues.length > 0 && result.issues.map((issue, i) => (
              ...
              {result.is_index && result.child_sitemaps && (
                <>
                  <h3 style={{ fontFamily: 'var(--heading-font)', fontWeight: 700, fontSize: '1.1rem', color: 'var(--primary-color)', margin: '24px 0 12px' }}>Child Sitemaps ({result.child_sitemaps.length})</h3>
              ...
              {pageUrls.length > 0 && (
                <>
                  <h3 style={{ fontFamily: 'var(--heading-font)', fontWeight: 700, fontSize: '1.1rem', color: 'var(--primary-color)', margin: '24px 0 12px' }}>
                    URLs {result.url_count && result.url_count > 100 ? `(showing first 100 of ${result.url_count})` : `(${urls.length})`}
                  </h3>
                  <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 16, overflow: 'hidden', boxShadow: 'var(--shadow-sm)' }}>
                    ...
                    {pageUrls.map((u, i) => (
                      <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 80px 80px', padding: '11px 20px', borderBottom: '1px solid var(--card-border)', alignItems: 'center' }}>
