...
interface TestResult { path: string; allowed_for_googlebot: boolean; allowed_for_all: boolean; }
interface RobotsResult {
  ...
  recommendation: string; error?: string; message?: string;
}
          ...
          <div className="row justify-content-center text-center">
            <div className="col-lg-8">
              <MotionWrapper variant="up">
                <p style={{ display: 'inline-block', background: 'rgba(245,158,11,0.08)', color: '#F59E0B', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 999, padding: '6px 18px', fontSize: '0.82rem', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 20 }}>Free Tool — No Sign-Up</p>
              ...
              <MotionWrapper variant="up" delay={0.1}>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', justifyContent: 'center', marginBottom: 12 }}>
                  <input type="url" value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && !loading && run()} placeholder="https://yourwebsite.com" disabled={loading}
                    ...
                    onFocus={e => (e.currentTarget.style.borderColor = '#F59E0B')} onBlur={e => (e.currentTarget.style.borderColor = 'var(--card-border)')} />
                  <input type="text" value={testPath} onChange={e => setTestPath(e.target.value)} placeholder="/optional-path-to-test" disabled={loading}
                    ...
                    onFocus={e => (e.currentTarget.style.borderColor = '#F59E0B')} onBlur={e => (e.currentTarget.style.borderColor = 'var(--card-border)')} />
                  <button onClick={run} disabled={loading || !url.trim()} style={{ padding: '15px 32px', background: loading ? 'var(--card-border)' : 'linear-gradient(135deg, #F59E0B, #EF4444)', color: '#fff', border: 'none', borderRadius: 10, font: '600 1rem var(--default-font)', cursor: loading ? 'wait' : 'pointer', opacity: loading || !url.trim() ? 0.7 : 1, whiteSpace: 'nowrap' }}>
            ...
            <MotionWrapper variant="up">
              ...
              <div style={{ padding: '24px 32px', background: result.found ? 'rgba(16,185,129,0.06)' : 'rgba(239,68,68,0.06)', border: `1px solid ${result.found ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}`, borderRadius: 16, marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  <i className={`fa ${result.found ? 'fa-circle-check' : 'fa-circle-xmark'}`} style={{ fontSize: '1.2rem', color: result.found ? 'var(--success-color)' : 'var(--error-color)' }} />
              ...
              {result.test_path_result && (
                <div style={{ padding: '20px 24px', background: 'var(--bg-secondary)', border: '1px solid var(--card-border)', borderRadius: 12, marginBottom: 24 }}>
                  <h3 style={{ fontFamily: 'var(--heading-font)', fontWeight: 700, fontSize: '0.95rem', color: 'var(--primary-color)', marginBottom: 12 }}>Path Test: <code style={{ fontWeight: 400 }}>{result.test_path_result.path}</code></h3>
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    {[
                      ...
                      <span key={b.label} style={{ padding: '6px 16px', borderRadius: 8, fontSize: '0.88rem', fontWeight: 600, background: b.allowed ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: b.allowed ? 'var(--success-color)' : 'var(--error-color)', border: `1px solid ${b.allowed ? 'rgba(16,185,129,0.3)' : 'rgba(239,68,68,0.3)'}` }}>
              ...
              {result.bot_access.length > 0 && (
                <>
                  <h3 style={{ fontFamily: 'var(--heading-font)', fontWeight: 700, fontSize: '1.1rem', color: 'var(--primary-color)', marginBottom: 16 }}>Search Engine Access</h3>
                  <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 16, overflow: 'hidden', marginBottom: 28, boxShadow: 'var(--shadow-sm)' }}>
                    {result.bot_access.map((b, i) => (
                      <div key={b.bot} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 24px', borderBottom: i < result.bot_access.length - 1 ? '1px solid var(--card-border)' : 'none' }}>
                        ...
                        <span style={{ padding: '4px 14px', borderRadius: 999, fontSize: '0.8rem', fontWeight: 700, background: b.allowed ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)', color: b.allowed ? 'var(--success-color)' : 'var(--error-color)' }}>
              ...
              <div className="row g-4">
                ...
                {result.rules.length > 0 && (
                  <div className="col-lg-6">
                    <h3 style={{ fontFamily: 'var(--heading-font)', fontWeight: 700, fontSize: '1rem', color: 'var(--primary-color)', marginBottom: 12 }}>Parsed Rules</h3>
                    {result.rules.slice(0, 10).map((r, i) => (
                      <div key={i} style={{ padding: '12px 16px', background: 'var(--bg-secondary)', borderRadius: 10, marginBottom: 8 }}>
                        ...
                        {r.disallows.map(d => <div key={d} style={{ fontSize: '0.82rem', color: 'var(--error-color)' }}>Disallow: {d}</div>)}
                        {r.allows.map(a => <div key={a} style={{ fontSize: '0.82rem', color: 'var(--success-color)' }}>Allow: {a}</div>)}
                ...
                <div className={result.rules.length > 0 ? 'col-lg-6' : 'col-lg-12'}>
                  ...
                  {result.crawl_delay !== null && (
                    <div style={{ marginTop: 16, padding: '14px 18px', background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: 10, fontSize: '0.9rem', color: 'var(--text-color)' }}>
                      <i className="fa fa-clock" style={{ color: '#F59E0B', marginRight: 8 }} />
                      Crawl-delay: <strong>{result.crawl_delay}s</strong> — crawlers will wait this long between requests.
                    </div>
                  )}
                </div>
              </div>
              ...
              {result.found && result.raw && (
                <div style={{ marginTop: 24 }}>
                  ...
                  {showRaw && <pre style={{ marginTop: 12, padding: '16px', background: 'var(--bg-secondary)', border: '1px solid var(--card-border)', borderRadius: 10, fontSize: '0.82rem', overflowX: 'auto', color: 'var(--text-muted)', lineHeight: 1.6, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>{result.raw}</pre>}
                </div>
              )}
            </MotionWrapper>
          </div>
